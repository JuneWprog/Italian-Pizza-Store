...
exports.register = [
    check('firstName')
        ...
        .isAlpha("en-US", { 'ignore': ' ' }).withMessage("First Name should be Alphabetic."),
    check('lastName')
        .isAlpha("en-US", { 'ignore': ' ' }).withMessage("Last Name should be Alphabetic."),
    check('email')
    ...
    async function (req, res) {
        try {
            ...
            const { firstName, lastName, address, phone, email, encryptedPassword } = req.body;

            if (!(email && encryptedPassword && firstName && lastName && address && phone)) {
                res.status(400).json({
                    "register": "All input is required"
                });
            }

            let decryptedPassword = CryptoJS.AES.decrypt(encryptedPassword, process.env.SECRET).toString(CryptoJS.enc.Utf8);
    ...
    async function (req, res) {
        try {
            ...
            if (user && (await bcrypt.compare(decryptedPassword, user.password))) {
                ...
                delete loggedInUser._doc.email;
